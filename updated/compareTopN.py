...
from helper_functions import give_dates
...
def plot_topN(final_result, allClients, number):
	...
	fig.add_trace(go.Bar(x=name_scatter, y=y_bar,textposition='outside'), row=2, col=1)
	fig.update_yaxes(title_text="Mean Successful<br>Prdicted Amt(USD)", row=2, col=1)
	fig.update_yaxes(title_text="Monthly Successful<br>Predicted Paid Amt(USD) ", row=3, col=1)
